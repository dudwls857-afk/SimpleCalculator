...
- ![과제4 실행화면] <img src="img/Cal5.png" alt="1단계 코드의 실행 스크린샷">
 <img src="img/Cal6.png" alt="1단계 코드의 실행 스크린샷">
- 과제 내용  
  ...
  - 계산기 프로그램에 괄호 ( ) 기능을 추가하여 사용자가 연산의 우선순위를 직접 설정할 수 있도록 한다.
- 구현 내용과 기능 설명  
  ...
  - 계산식 입력 문자열에 괄호를 포함하여 처리할 수 있도록 수정하였다.
  - 괄호가 포함된 수식을 계산하기 위해 문자열을 분석하여 올바른 수식 형태로 변환하였다.
  - 잘못된 괄호 입력 시 예외가 발생하지 않도록 오류 처리 기능을 추가하였다.
  -사용자가 ( )를 입력하면 해당 괄호 내부의 연산이 먼저 계산된다.
  -예: 2 + (3 × 4) 입력 시, 괄호 안의 연산이 먼저 수행되어 14가 출력된다.
  -올바르지 않은 괄호 입력 시 계산이 되지 않고 오류를 방지하도록 처리된다.
...
추가로 계산기 프로그램을 구현하면서 문자열 형태의 수식을 처리하는 방법을 이해하게 되었다.
특히 괄호 ( ) 기능을 추가하는 과정에서 연산의 우선순위 개념을 명확히 익힐 수 있었다.
잘못된 입력으로 인해 발생하는 오류를 방지하기 위해 예외 처리의 중요성을 알게 되었고, 안정적인 프로그램을 만드는 방법을 배웠다.
여러 연산 기호가 포함된 수식을 처리하면서 입력값을 분석하고 분리하는 로직 설계 능력을 향상시킬 수 있었다.
또한 기존 기능에 새로운 기능을 추가할 때 코드 구조를 고려해야 유지보수가 쉽다는 점을 경험하였다.
